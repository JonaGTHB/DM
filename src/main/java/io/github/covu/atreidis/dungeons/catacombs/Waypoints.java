...
import io.github.covu.atreidis.utils.WaypointUtils;
import io.github.covu.atreidis.events.PacketEvent;
...
public class Waypoints {
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        if (!enabled) return;
        if (practiceModeOn && !Atreides.keyBindings[2].isKeyDown()) return;
        String roomName = RoomDetection.roomName;
        if (roomName.equals("undefined") || Atreides.roomsJson.get(roomName) == null || secretsList == null) return;
        if (Atreides.waypointsJson.get(roomName) != null) {
            JsonArray secretsArray = Atreides.waypointsJson.get(roomName).getAsJsonArray();
                        ...
                        if (pos.equals(event.pos)) {
                            for(int j = 1; j <= secretNum; j++) {
                                if (secretsObject.get("secretName").getAsString().substring(0,2).replaceAll("[\\D]", "").equals(String.valueOf(j))) {
                                    ...
                                    Atreides.logger.info("DungeonRooms: Detected " + secretsObject.get("category").getAsString() + " click, turning off waypoint for secret #" + j);
                                    break;
                                }
                            }
                        }
                                ...
                                if (entity.getDistanceSq(pos) <= 36D) {
                                    for(int j = 1; j <= secretNum; j++) {
                                        if (secretsObject.get("secretName").getAsString().substring(0,2).replaceAll("[\\D]", "").equals(String.valueOf(j))) {
                                            ...
                                            Waypoints.secretsList.set(j-1, false);
                                            Waypoints.allSecretsMap.replace(roomName, Waypoints.secretsList);
                                            Atreides.logger.info("DungeonRooms: " + entity.getCommandSenderEntity().getName() + " picked up " +  StringUtils.stripControlCodes(name) + " from a "  + secretsObject.get("category").getAsString() + " secret, turning off waypoint for secret #" + j);
                                            return;
                                        }
                                    }
                                }
                            ...
                            if (player.getDistanceSq(pos) <= 16D) {
                                for(int j = 1; j <= secretNum; j++) {
                                    if (secretsObject.get("secretName").getAsString().substring(0,2).replaceAll("[\\D]", "").equals(String.valueOf(j))) {
                                        ...
                                        Waypoints.secretsList.set(j-1, false);
                                        Waypoints.allSecretsMap.replace(roomName, Waypoints.secretsList);
                                        Atreides.logger.info("DungeonRooms: Player sneaked near " + secretsObject.get("category").getAsString() + " secret, turning off waypoint for secret #" + j);
                                        return;
                                    }
                                }
                            }
