...
package io.github.covu.atreidis.utils;

import io.github.covu.atreidis.Atreides;
import io.github.covu.atreidis.handlers.ScoreboardHandler;
...
public class Utils {
    ...
    public static void checkForConflictingHotkeys() {
        Minecraft mc = Minecraft.getMinecraft();
        for (KeyBinding drmKeybind : Atreides.keyBindings) {
    ...
    public static List<Path> getAllPaths (String folderName) {
        ...
        try {
            URI uri = Atreides.class.getResource("/assets/dungeonrooms/" + folderName).toURI();
            Path Path;
    ...
    public static HashMap<String, long[]> pathsToRoomData (String parentFolder, List<Path> allPaths) {
        ...
        try {
            for (Path path : allPaths) {
                ...
                String name = path.getFileName().toString();
                InputStream input = Atreides.class.getResourceAsStream(path.toString());
                ...
                Atreides.logger.debug("DungeonRooms: Loaded " + name);
            }
        } catch (IOException | ClassNotFoundException e) {
            e.printStackTrace();
        }
        Atreides.logger.info("DungeonRooms: Loaded " + allRoomData.size() + " " + parentFolder + " rooms");
        return allRoomData;
    }
