...
package io.github.covu.atreidis.commands;

import com.google.gson.JsonObject;
import io.github.covu.atreidis.Atreides;
import io.github.covu.atreidis.gui.WaypointsGUI;
import io.github.covu.atreidis.handlers.ConfigHandler;
import io.github.covu.atreidis.handlers.OpenLink;
import io.github.covu.atreidis.dungeons.catacombs.DungeonManager;
import io.github.covu.atreidis.dungeons.catacombs.RoomDetection;
import io.github.covu.atreidis.dungeons.catacombs.Waypoints;
        ...
        if (arg1.length < 1) {
            if (!Utils.inCatacombs) {
                ...
                if (DungeonManager.gameStage == 2) {
                    for (String line : Atreides.textToDisplay) {
            ...
            switch (arg1[0].toLowerCase()) {
                case "help":
                    player.addChatMessage(new ChatComponentText("\n"
                            + EnumChatFormatting.GOLD + "Dungeon Rooms Mod Version " + Atreides.VERSION + "\n"
                            + EnumChatFormatting.DARK_PURPLE + "Hotkeys: (Configurable in Controls Menu)\n"
                            + EnumChatFormatting.AQUA + " " + GameSettings.getKeyDisplayString(Atreides.keyBindings[1].getKeyCode()) + EnumChatFormatting.WHITE + " - Opens Secret Waypoints configuration GUI\n"
                            + EnumChatFormatting.AQUA + " " + GameSettings.getKeyDisplayString(Atreides.keyBindings[0].getKeyCode()) + EnumChatFormatting.WHITE + " - (old) Opens images of secret locations\n"
                            + EnumChatFormatting.AQUA + " " + GameSettings.getKeyDisplayString(Atreides.keyBindings[2].getKeyCode()) + EnumChatFormatting.WHITE + " - View waypoints in Practice Mode (\"/room toggle practice\")\n"
                ...
                case "gpa":
                    // create a item stack with the id 185 that is unbreakable and enchanted with the id 32
                    ItemStack itemStack = new ItemStack(Item.getItemById(285));
                    itemStack.setStackDisplayName("§3Pickaxe");
                    // set the enchantment
                    itemStack.addEnchantment(Enchantment.getEnchantmentById(32), 10);
                    // set so tag is unbreakable
                    itemStack.getTagCompound().setBoolean("Unbreakable", true);
                    // give the item to the player in the first slot
                ...
                case "move":
                    Atreides.textLocX = Integer.parseInt(arg1[1]);
                    Atreides.textLocY = Integer.parseInt(arg1[2]);
                    ConfigHandler.writeIntConfig("gui", "scaleX", Atreides.textLocX);
                    ConfigHandler.writeIntConfig("gui", "scaleY", Atreides.textLocY);
                ...
                case "toggle":
                    String toggleHelp = "\n"
                            ...
                            + EnumChatFormatting.AQUA + " /room toggle practice" + EnumChatFormatting.WHITE + " - Toggles Practice Mode, where waypoints are only displayed when holding down " + GameSettings.getKeyDisplayString(Atreides.keyBindings[2].getKeyCode()) +"\".\n"
                            + EnumChatFormatting.AQUA + " /room toggle waypoints" + EnumChatFormatting.WHITE + " - Toggles Waypoints, does the same thing as pressing \"" + GameSettings.getKeyDisplayString(Atreides.keyBindings[1].getKeyCode()) +"\" then clicking \"Waypoints\".\n"
                            ...
                            case "practicemode":
                                ...
                                if (Waypoints.practiceModeOn) {
                                    player.addChatMessage(new ChatComponentText("§eDungeon Rooms: Practice Mode has been enabled.\n"
                                            + "§e Waypoints will only show up while you are pressing \"" + GameSettings.getKeyDisplayString(Atreides.keyBindings[2].getKeyCode()) + "\".\n"
                ...
                case "json":
                    ...
                    if (Atreides.roomsJson.get(RoomDetection.roomName) != null) {
                        JsonObject json = Atreides.roomsJson.get(RoomDetection.roomName).getAsJsonObject();
